...
describe("deleteCompleted()", () => {
  test("Should remove all completed tasks from the list", () => {
    const todos = createMockTodos(); // This mock has 2 completed and 2 incomplete tasks
    Todos.deleteCompleted(todos);
    expect(todos).toHaveLength(2);
    expect(todos.every((t) => t.completed === false)).toBe(true);
  });

  test("Should not change the list if no tasks are completed", () => {
    const todos = [{ task: "Task A", completed: false }];
    const before = [...todos];
    Todos.deleteCompleted(todos);
